...
import { request, extend, clone } from '../utils'

export type ApiMethod<
  T,
  U extends ApiMethodConfigType = ApiMethodConfigType,
> = {
  [P in keyof T]: U
}
...
export class ApiModel<T> {
  ...
  constructor(
    protected apiConfig: ApiConfigModel<ApiMethod<T>>,
    opt?: {
      beforeRequest?: BeforeRequest
  ...
  protected async requestByConfig<U = any>(
    config: ApiMethodConfigType,
    options?: RequestByConfigOption<U>,
  ) {
...
export type ApiMethodConfigType<
  T extends ApiDefaultArgsType = ApiDefaultArgsType,
> = {
  url: string
  method?: string
